...
(function () {
  ...
  /**
   * Render page (1-based) to PNG data URL.
   * @param {number} scale  — render scale (2 = ~144–150 DPI look)
   */
  ...
  /**
   * Ask user which page if multi-page; returns data URL result or null if cancelled.
   */
  async function pickAndRender(pdf, preferredPage = 1) {
    ...
    // High-res render for accurate takeoff (scale 2.5 ≈ 180 DPI on 72pt base)
  ...
  /**
   * Load every page as separate rendered images (for multi-page import).
   * Caps at maxPages to avoid huge memory use.
   */
